...
n <- 1
plot.name <- "Plots/figExt03_prey_trajectories/figExt03.pdf"
pdf(plot.name, height=(4.5+1)/5, width=4.5+1, useDingbats = FALSE)
par(mfrow= c(1, 5), oma=c(0, 0, 0, 0), mar=c(0, 0, 1, 0))

create_graph = FALSE
if(file.exists("saved/figExt03_trajectories.rdata")){
  load(file="saved/figExt03_trajectories.rdata")
} else{
  create_graph = TRUE
  world.trajectories = vector("list", length(5))
}

for (visualrange in 1:5){
...
if(create_graph){
  save(world.trajectories, file="saved/figExt03_trajectories.rdata")
}
dev.off()